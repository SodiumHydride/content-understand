...
def _ensure_local_ready(
    ...
    *,
    ...
    if not find_app_binary(runtime_dir):
        if on_progress:
            on_progress("setup", 5, "Downloading app Ollama...")
        download_ollama(runtime_dir, on_progress=on_progress, github_mirror=config.github_mirror)

    if on_progress:
        on_progress("setup", 10, "Starting app Ollama...")

    daemon = get_shared_daemon()
    try:
        base = daemon.start(runtime_dir, models_dir(), ollama_mirror=config.ollama_mirror)
        rt.mark_ollama_ready(base, "app")
        return base
    except Exception as exc:
...
def _build_pipeline(config: ContentConfig, on_progress: ProgressFn | None = None):
    ...
    # Apply proxy from config to environment so downloaders/resolvers pick it up
    if config.http_proxy:
        os.environ.setdefault("http_proxy", config.http_proxy)
        os.environ.setdefault("https_proxy", config.http_proxy)
        os.environ.setdefault("HTTP_PROXY", config.http_proxy)
        os.environ.setdefault("HTTPS_PROXY", config.http_proxy)
    ...
    local_needed = any(getattr(config, f"{ct}_backend", "") == "local_server" for ct in _KINDS)
    if local_needed:
        from engine.runtime.manager import get_runtime_manager
        ...
        elif any(getattr(config, f"{ct}_backend", "") == "local_server" for ct in _KINDS):
            raise RuntimeError(
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            result = get_plugin_manager().run_post_understand(result)
        except Exception as pe:
            logger.error("Plugin execution failed: %s", pe)
...
def _validate_ingest_url(url: str) -> None:
    ...
    if (
        ...
        and "b23.tv" not in host
        and "BV" not in url
        and "/av" not in path.lower()
    ):
        raise ValueError("Unrecognized Bilibili URL. Use a /video/BV… link or a b23.tv short link.")
